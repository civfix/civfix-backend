...
```sh
sudo -n docker exec "compose-api-$(sudo -n cat /opt/civfix/state/active-color)-1" node dist/db/backfill-user-activity.js
```
...
```sh
sudo -n docker exec "compose-api-$(sudo -n cat /opt/civfix/state/active-color)-1" node dist/db/backfill-post-geom.js
```
