 ...
 */
describe.skipIf(!pg)("report message mapper surfaces forwardedToCity + cityMention (integration)", () => {
  let h: PgHarness
  ...
  it("reportHistory reports forwardedToCity:true + a forwarded cityMention once a matching forward is stamped", async () => {
    ...
    expect(found!.cityMention).toMatchObject({ handle: HANDLE, geoid, name: "City of San Francisco", forwarded: true })
