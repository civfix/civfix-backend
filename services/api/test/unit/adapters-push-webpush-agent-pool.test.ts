// The pool exists for SSRF pinning, but its eviction policy is what these tests pin: the first version
// destroyed EVERY cached agent at the ceiling, and node's Agent.destroy() tears down mid-request sockets,
// so one eviction during a fan-out aborted the other in-flight pushes (which are never retried).

import { describe, it, expect, vi } from "vitest"
import { makePinnedAgentPool } from "../../src/adapters/push-webpush.js"
import type { Agent } from "node:https"

// The stub socket carries a destroy() because Agent.destroy() calls it on every socket, which is the
// whole bug: destroying the agent destroys the socket under the request using it.
...
// Node deletes the bucket when the last socket goes.
...
describe("makePinnedAgentPool", () => {
  it("caches one keep-alive agent per address+family", () => {
    ...
    try {
      ...
      // Without keepAlive the cache buys nothing. @types/node does not surface `keepAlive`.
      const opts = a as unknown as { keepAlive?: boolean; options: { timeout?: number } }
      expect(opts.keepAlive).toBe(true)
      // An idle-socket ceiling, so a quiet pinned address does not hold a socket forever.
