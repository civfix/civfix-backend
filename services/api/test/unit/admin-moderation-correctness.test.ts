...
describe("owner takedown requests", () => {
  ...
  it("folding into an open item keeps the owner-consent marker and flag", async () => {
    const ctl = makeFakeSql([
    ...
    expect(escalate?.values).toContainEqual({ ownerTakedown: true })
    expect(escalate?.values).toContain("Owner takedown request")
  })
