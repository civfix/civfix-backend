...
/**
 * M(bounce spoofing): `handleBounce` used to accept a DSN from ANY sender. The only binding was knowledge
 * of an outbound Message-ID plus the failed recipient address — both present in every packet civfix
 * forwards — so a recipient-side party (a CC'd vendor, a records portal) could stamp `bounced_at` on a
 * jurisdiction contact and suppress routing for that whole geoid.
 *
 * RULE: a DSN is only believed when its From domain is aligned with the domain of the address that
 * failed (the receiving MTA reporting for its own domain) or with our own sending domain (our provider
 * generating the DSN), and never when the auth verdict is a hard `fail`. `unknown` is accepted because a
 * real DSN has a null envelope sender, so SPF/DKIM — and therefore DMARC — are routinely absent.
 */
