...
// H12: authenticated RSVP (join/leave) is a mutation on a shared row, so it carries an explicit bucket
// like every other RSVP surface (guest request/verify/cancel, claimEventSlot). Keyed per identity —
// rotating egress must not buy a fresh budget — with the standard host ceiling above it.
