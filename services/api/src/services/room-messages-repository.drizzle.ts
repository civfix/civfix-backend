 ...
 * everything dm-only) deliberately stays in the chat and dm repositories. Identifiers render through `sql(...)` over
...
export function makeDrizzleRoomMessagesRepository<Row extends RoomScopeRow, Ctx>(
  ...
  async function history(
    ...
    if (around !== undefined) return historyAround(around, limit, viewerUserId)
  ...
  async function setPinned(
    ...
    `
    return findMessage(messageId, userId)
  }

  async function listPins(viewerUserId: string | null): Promise<ChatMessageDTO[]> {
    const alias = sql(spec.alias)
    const [rows, ctx] = await Promise.all([
      sql<Row[]>`
        SELECT ${spec.columns}
  ...
  return { history, historyAround, findMessage, setPinned, listPins }
}
