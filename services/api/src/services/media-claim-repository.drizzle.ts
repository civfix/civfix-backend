...
export async function findClaimableAvatarIn(
  tx: Queryable,
  uploadId: string,
  claimant: AvatarClaimant,
): Promise<AvatarMediaRow[]> {
  const rows = await avatarClaimQuery(
    (strings, ...values) => tx<AvatarMediaRow[]>(strings, ...values),
    uploadId,
    claimant,
  )
  return rows
...
export async function lockUploadsForClaimIn(tx: Queryable, uploadIds: readonly string[]) {
  await tx`
